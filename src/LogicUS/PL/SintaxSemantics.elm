...
@docs splToString, splToMathString, splTruthTableString, splTruthTableMathString


# Other functions

@docs interpretationsFromSymbolsAndLiterals
...
{-| It reads the formula from a string. It returns a tuple with may be a formula (if it can be read it), the input considerated to parse and a message of error it it is not able to performs the parsing.
    ...
    fplReadFromString "_|_" == ( Just Insat, "_|_", "" )
...
{-| It generates the Latex string of a PL formula. The result requires a math enviroment to be displayed.
    ...
    fplToMathString f1 == "\\left( a\\rightarrow b \\right)"

    fplToMathString f2 == "\\left( \\neg \\left( a \\wedge b \\right)\\leftrightarrow \\left( \\neg a \\vee \\neg b \\right) \\right)"
...
fplToMathString f =
    case f of
        ...
        Conj p q ->
            "\\left( " ++ fplToMathString p ++ " \\wedge " ++ fplToMathString q ++ " \\right)"

        Disj p q ->
            "\\left( " ++ fplToMathString p ++ " \\vee " ++ fplToMathString q ++ " \\right)"

        Impl p q ->
            "\\left( " ++ fplToMathString p ++ "\\rightarrow " ++ fplToMathString q ++ " \\right)"

        Equi p q ->
            "\\left( " ++ fplToMathString p ++ "\\leftrightarrow " ++ fplToMathString q ++ " \\right)"
...
{-| It generates the Latex string of a Set of PL formulas. The result requires a math enviroment to be displayed.
    ...
    splToMathString [ f1, f2 ] == "\\left\\lbrace \\left( a\\rightarrow b \\right), \\left( \\neg \\left( a \\wedge b \\right)\\leftrightarrow \\left( \\neg a \\vee \\neg b \\right) \\right)\\right\\rbrace"

-}
splToMathString : List FormulaPL -> String
splToMathString fs =
    "\\left\\lbrace " ++ (String.join ", " <| List.map fplToMathString fs) ++ "\\right\\rbrace"
...
splTruthTableString fs =
    ...
    let
        head =
            symbs ++ List.map fplToString fs ++ [ "U" ]
...
splTruthTableMathString fs =
    ...
    let
        head =
            symbs ++ List.map fplToMathString fs ++ [ "U" ]
...
fplFormTreeToDOT ftree =
    ...
    in
    GDOT.outputWithStyles myStyles (\x -> Just (fplToString x)) (\_ -> Nothing) ftree
